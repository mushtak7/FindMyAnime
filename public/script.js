...
function cacheResponse(key, data, ttl = 300000) {
  ...
  try {
    const lsKey = 'fma_cache_' + simpleHash(key);
    localStorage.setItem(lsKey, JSON.stringify({ data, expires: Date.now() + ttl }));
  ...
  try {
    ...
    if (stored) {
      ...
      if (parsed.expires > Date.now()) {
        appState.cache.set(key, parsed); // Promote back to memory
        return parsed.data;
      }
...
function createErrorState(message, retryCallback) {
  const div = document.createElement('div');
  div.className = 'error-state-container';
  div.style.cssText = 'text-align:center; padding:40px 20px; color:var(--muted,#888);';
  div.innerHTML = `
    <div style="font-size:48px; margin-bottom:12px;">😵</div>
    <h3 style="margin:0 0 8px; color:#e0e0e0; font-size:1.1rem;">${message || 'We hit a snag'}</h3>
    <p style="margin:0 0 16px; font-size:0.85rem;">Check your connection or try again.</p>
    ${retryCallback ? '<button class="retry-btn" style="padding:10px 24px; background:#3b82f6; color:white; border:none; border-radius:50px; cursor:pointer; font-weight:600; transition:transform 0.2s;">Try Again</button>' : ''}
  `;
  if (retryCallback) {
    ...
    btn.onclick = () => {
      div.replaceWith(createSkeletonCard());
      retryCallback();
    };
  }
  return div;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadAllData() {
    try {
      ...
      idleCallback(() => loadCommunityBuzz());

    } catch (e) { console.error('Error loading data:', e); }
  }

  // Community Buzz
  async function loadCommunityBuzz() {
    const container = getElement('communityBuzz');
    if (!container) return;
    try {
      const res = await fetch(`${API_BASE}/api/feed/recent`);
      ...
      if (data.recentReviews && data.recentReviews.length) {
        data.recentReviews.slice(0, 3).forEach(r => {
          items.push(`
            <div style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.06); border-radius:12px; padding:16px; cursor:pointer; transition: all 0.3s;" 
                 onclick="location.href='/${r.target_type === 'manga' ? 'manga-details' : 'anime'}.html?id=${r.target_id}'"
              ...
              <div style="display:flex; align-items:center; gap:8px; margin-bottom:8px;">
                <span style="background:rgba(255,193,7,0.15); color:#ffd54f; padding:2px 8px; border-radius:999px; font-size:10px; font-weight:600;">⭐ Review</span>
                <span style="font-size:12px; font-weight:600;">${escapeHtml(r.username)}</span>
                <span style="margin-left:auto; font-size:11px; color:var(--muted);">${new Date(r.created_at).toLocaleDateString()}</span>
              </div>
              <div style="color:#ffd700; font-size:12px; margin-bottom:4px;">${'⭐'.repeat(r.rating)}</div>
              <div style="font-size:13px; color:#ccc; line-height:1.5; overflow:hidden; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;">${escapeHtml(r.comment || '')}</div>
            </div>
          `);
        });
      }
      ...
      if (data.recentPosts && data.recentPosts.length) {
        const catIcons = { discussion: '💬', review: '⭐', recommendation: '🎯', question: '❓', meme: '😂' };
        data.recentPosts.slice(0, 3).forEach(p => {
          items.push(`
            <div style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.06); border-radius:12px; padding:16px; cursor:pointer; transition: all 0.3s;"
                 ...
                 onmouseout="this.style.borderColor='rgba(255,255,255,0.06)'; this.style.transform='none'">
              <div style="display:flex; align-items:center; gap:8px; margin-bottom:8px;">
                <span style="background:rgba(33,150,243,0.15); color:#64b5f6; padding:2px 8px; border-radius:999px; font-size:10px; font-weight:600;">${catIcons[p.category] || '💬'} ${p.category}</span>
                <span style="font-size:12px; font-weight:600;">${escapeHtml(p.username)}</span>
                <span style="margin-left:auto; font-size:11px; color:var(--muted);">${new Date(p.created_at).toLocaleDateString()}</span>
              </div>
              <div style="font-size:13px; color:#ccc; line-height:1.5; overflow:hidden; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;">${escapeHtml(p.content || '')}</div>
            </div>
          `);
        });
      }
      ...
      if (items.length > 0) {
        ...
        container.innerHTML = '<div style="text-align:center; grid-column:1/-1; padding:30px; color:var(--muted); font-size:14px;">No community activity yet. Be the first to post!</div>';
      }
    } catch (e) {
      container.innerHTML = '<div style="text-align:center; grid-column:1/-1; padding:30px; color:var(--muted); font-size:14px;">Community buzz will appear here</div>';
    }
  }
  ...
  async function loadSection(id, url) {
    ...
    try {
      ...
      box.replaceChildren(createErrorState('Failed to load this section', () => loadSection(id, url)));
    }
  }
...
async function loadSchedule(day) {
  ...
  // Fetch user watchlist IDs for cross-reference
  let trackedIds = new Set();
  try {
    const wlRes = await fetch('/api/watchlist', { credentials: 'include' });
    if (wlRes.ok) {
      const wlData = await wlRes.json();
      wlData.forEach(item => trackedIds.add(item.anime_id || item));
    }
  ...
  try {
    ...
    const dayIndex = validDays.indexOf(normalizedDay);
    // Map: monday=0 -> JS day 1, etc.
    const jsDayMap = [1, 2, 3, 4, 5, 6, 0]; // mon=1, tue=2, ..., sun=0
    const targetJsDay = jsDayMap[dayIndex];
    ...
    data.forEach(anime => {
      ...
      const trackBadge = isTracked
        ? '<span style="display:inline-block;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;background:rgba(76,175,80,0.2);color:#81c784;margin-left:6px;">Tracking ✓</span>'
        : '';

      // Compute local time and countdown from JST broadcast time
      let localTimeStr = '';
      let countdownStr = '';
      const broadcastTime = anime.broadcast?.time; // e.g. "17:00"
      if (broadcastTime && /^\d{2}:\d{2}$/.test(broadcastTime)) {
        const [h, m] = broadcastTime.split(':').map(Number);
        // Construct next broadcast in JST (UTC+9), then convert to local
        const now = new Date();
        ...
        target.setDate(now.getDate() + (diff === 0 && now.getHours() * 60 + now.getMinutes() > (h - 9 + now.getTimezoneOffset() / 60) * 60 + m ? 7 : diff));

        // Set to JST broadcast time, converted to UTC
        target.setUTCHours(h - 9, m, 0, 0);
        if (target < now) target.setDate(target.getDate() + 7);

        const localOptions = { hour: '2-digit', minute: '2-digit', hour12: true };
        localTimeStr = target.toLocaleTimeString(undefined, localOptions);
        ...
        if (msLeft > 0) {
          ...
          if (hrsLeft < 24) {
            countdownStr = `<span style="font-size:10px;color:#ffd54f;font-weight:600;">⏳ ${hrsLeft}h ${minsLeft}m</span>`;
          } else {
            const daysLeft = Math.floor(hrsLeft / 24);
            countdownStr = `<span style="font-size:10px;color:var(--muted);">${daysLeft}d left</span>`;
          }
        }
      }
      ...
      div.innerHTML = `
        ...
        <div class="schedule-info">
          <div class="time-badge">⏰ ${broadcastTime || 'TBA'} JST${localTimeStr ? ` <span style="font-size:10px;color:var(--muted);">(${localTimeStr})</span>` : ''} ${countdownStr}</div>
          <div class="schedule-title">${anime.title || 'Unknown'}${trackBadge}</div>
          <div class="schedule-meta">${(anime.genres || []).slice(0, 2).map(g => g.name).join(', ') || 'N/A'} · ⭐ ${anime.score || 'N/A'}</div>
        </div>
      `;
      fragment.appendChild(div);
    });
