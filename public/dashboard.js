...
async function jikanFetch(url, retries = 3) {
    ...
    if (res.status === 429) {
        if (retries <= 0) throw new Error('Jikan rate limit exceeded');
...
function makeCard(anime, badgeType) {
    ...
    return `<div class="card-anime" onclick="window.location.href='/anime.html?id=${malId}'">
        <div class="card-img">
            ...
            <img src="${img}" alt="${escapeHtml(title)}" loading="lazy" decoding="async" width="180" height="240">
...
async function loadTrending() {
    ...
    try {
        ...
        if (heroCards.length > 0 && animeList.length >= 5) {
            heroCards.forEach((card, i) => {
                if (i < 5) {
                    ...
                    if (imgEl) {
                        imgEl.src = animeList[i].images?.jpg?.image_url || '';
        ...
        el.innerHTML = `<div style="text-align:center;padding:30px;color:var(--muted)">
            ...
            <button onclick="loadTrending()" style="margin-top:8px;padding:8px 20px;background:#3b82f6;color:white;border:none;border-radius:50px;cursor:pointer;font-weight:600">Try Again</button>
        </div>`;
    }
}
    ...
    el.innerHTML = big3Data.map(b => `
        <div class="big3-card" onclick="window.location.href='/anime.html?id=${b.id}'" style="cursor:pointer">
            <div class="big3-header">
                <img src="" alt="${b.title}" loading="lazy" decoding="async">
                <div>
                    <h3>${b.title}</h3>
...
async function loadTopRated() {
    ...
    try {
        ...
        el.innerHTML = `<div style="text-align:center;padding:30px;color:var(--muted)">
            <div style="font-size:36px;margin-bottom:8px">😵</div>
            <p>Could not load top rated</p>
            <button onclick="loadTopRated()" style="margin-top:8px;padding:8px 20px;background:#3b82f6;color:white;border:none;border-radius:50px;cursor:pointer;font-weight:600">Try Again</button>
        </div>`;
    }
}
...
async function loadGOAT() {
    ...
    try {
        ...
        el.innerHTML = `<div style="text-align:center;padding:30px;color:var(--muted)">
            <div style="font-size:36px;margin-bottom:8px">😵</div>
            <p>Could not load GOAT list</p>
            <button onclick="loadGOAT()" style="margin-top:8px;padding:8px 20px;background:#3b82f6;color:white;border:none;border-radius:50px;cursor:pointer;font-weight:600">Try Again</button>
        </div>`;
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load trending FIRST — it sets hero/LCP images
    loadTrending();

    // Defer non-critical loads to break up long main-thread tasks
    await delay(400);
    loadStats();
    await delay(400);
    loadBig3();

    // Below-fold sections loaded via requestIdleCallback
    const deferLoad = (fn, fallbackMs) => {
    ...
    deferLoad(() => loadTopRated(), 1000);
    deferLoad(() => loadGOAT(), 1500);
    deferLoad(() => loadFanFav(), 2000);
    deferLoad(() => loadManga(), 2500);
    deferLoad(() => loadCommunity(), 3000);
